...
def consumer_job(args, config, responses_queue=None):
    ...
    commun_user_interf_consumer = Consumer(config)

    # Set up a callback to handle the '--reset' flag.
    def reset_offset(commun_user_interf_consumer, partitions):
        if args.reset:
            for p in partitions:
                p.offset = OFFSET_BEGINNING
            commun_user_interf_consumer.assign(partitions)

    # Subscribe to topic
    topic = "commun_user_interf"
    commun_user_interf_consumer.subscribe([topic], on_assign=reset_offset)

    # Poll for new messages from Kafka and print them.
    try:
        while True:
            msg = commun_user_interf_consumer.poll(1.0)
            if msg is None:
        ...
        commun_user_interf_consumer.close()
