...
### Цели безопасности
1. Выполняются команды только от авторизованных пользователей.
...
3. Временные показатели частоты сердечного ритма и другая информация о состоянии пациента, хранящаяся в библиотеке данных, достоверны и не противоречивы.
4. Доступ к конфиденциальной информации ЭКС имеют только авторизованные пользователи.
...
## Тесты
Тесты находятся в папке tests. Для их запуска можно воспользоваться командой `make test`, находясь в директории code.

Можно запустить только один кокретный тест `pytest -svk <name>`, где вместо `<name>` нужно вставить название необходимого теста.

Представлены следующие тесты:
* юнит тесты политик безопасности (tests/unit/monitor/test_unit_monitor.py)
* сквозные тесты (tests/e2e/test_overall.py)

Сковзные тесты содержат проверку сценария стимуляции сердца, а также операции записи команд.
